...
#if defined(__PSSL__) && !defined(reversebits)
#define UNDEF_REVERSE_BITS
...
namespace STL
{
    ...
    namespace Color
    {
        ...
        {
            float luma = STL::Color::Luminance( color );
    ...
    */
    ...
    namespace BRDF
    {
        ...
        void DirectLightingLeaves( float3 N, float3 L, float3 V, float3 Rf0, float linearRoughness, out float3 Cdiff, out float3 Cspec )
        {
            float3 H = normalize( L + V );

            float NoL = ( dot( N, L ) );	// leave sun shading from DL2?
			if (-NoL > 0) {
				NoL = -NoL;
			}
			
			NoL = saturate(NoL * 0.5 + 0.5);
			NoL *= NoL;
			NoL = (NoL * NoL)*(3 - 2*NoL*NoL);
			NoL = min(NoL, 1.0);
...
#if defined(UNDEF_REVERSE_BITS)
#undef UNDEF_REVERSE_BITS
#undef reversebits
#endif
