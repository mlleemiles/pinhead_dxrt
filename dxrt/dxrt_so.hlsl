...
void ENTRY_POINT(raygeneration_main)()
{
    ...
    while( sampleNum < ZERO_TROUGHPUT_SAMPLE_NUM && throughput < 0.001 )
#endif
    {
    ...
    if (isHitFound)
    {
        float so = REBLUR_FrontEnd_GetNormHitDist(screenSpaceHitDist, z, spec_hit_dist_params, roughness);
        WriteCheckerboardOutput(pixelPos, so);
        return;
    }
    ...
    float so = REBLUR_FrontEnd_GetNormHitDist(payload.hitDist, z, spec_hit_dist_params, roughness);
    WriteCheckerboardOutput(pixelPos, so);
}
